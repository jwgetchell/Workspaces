...
Begin VB.Form frmMain 
   ...
   Begin VB.Frame f29177 
      ...
      Begin VB.Frame Frame4 
         ...
         End
      End
      Begin VB.CheckBox cb177CmpPrxOff 
   ...
   Begin VB.Frame Frame3 
      ...
      Top             =   6120
      Width           =   975
      Begin VB.TextBox tb58334aMux 
         Height          =   285
         Left            =   120
         MultiLine       =   -1  'True
         TabIndex        =   93
         Text            =   "frmMain.frx":0CE8
         Top             =   240
         Width           =   735
      End
   End
   Begin VB.Frame Frame2 
      ...
      Begin VB.ComboBox cmbDev 
         Height          =   315
         ItemData        =   "frmMain.frx":0CED
         Left            =   120
         List            =   "frmMain.frx":0D51
         TabIndex        =   91
         Text            =   "0"
         Top             =   480
         Width           =   735
      End
      Begin VB.TextBox tbI2Caddr 
         Height          =   285
         Left            =   120
         MultiLine       =   -1  'True
         TabIndex        =   90
         Text            =   "frmMain.frx":0DCB
         Top             =   240
         Width           =   735
      End
   End
   ...
   Begin VB.Frame fmRegisters 
      ...
      Begin VB.ComboBox cmbIO 
         Height          =   315
         Index           =   0
         ItemData        =   "frmMain.frx":0DD4
         Left            =   120
         List            =   "frmMain.frx":0DF0
         TabIndex        =   82
         Text            =   "0"
         Top             =   720
         Width           =   615
      ...
      Begin VB.ComboBox cmbIO 
         Height          =   315
         Index           =   1
         ItemData        =   "frmMain.frx":0E0C
         Left            =   720
         List            =   "frmMain.frx":0E28
         TabIndex        =   81
         Text            =   "FF"
         Top             =   720
         Width           =   615
   ...
   Begin VB.Frame fmTimers 
      ...
      End
   End
   Begin Application.ucALSusb ucALSusb1 
      Height          =   5895
      Left            =   0
      TabIndex        =   45
      Top             =   0
      ...
      Begin VB.Frame frmDataStats 
         ...
         Begin VB.Frame fnSweep 
            ...
            Begin VB.ComboBox cmbSweep 
               Height          =   315
               ItemData        =   "frmMain.frx":0E4C
               Left            =   120
               List            =   "frmMain.frx":0E5F
               TabIndex        =   19
               Text            =   "nm"
               Top             =   240
               Width           =   1095
...
Public Sub cb177CmpPrxOff_Click(Index As Integer)
    ...
    If Index = 0 Then
        If cb177CmpPrxOff(Index).value = vbChecked Then
            ucALSusb1.dSetPrxRngOffCmpEn 1
            cb177CmpPrxOff(Index).caption = "PrxOffCmp:On"
        Else
            cb177CmpPrxOff(Index).caption = "PrxOffCmp:Off"
            ucALSusb1.dSetPrxRngOffCmpEn 0
        End If
    Else
    
        If cb177CmpPrxOff(Index).value = vbChecked Then
        
            cb177CmpPrxOff(Index).enabled = False
    
            If cb177CmpPrxOff(0).value <> vbChecked Then
                cb177CmpPrxOff(0).value = vbChecked
                cb177CmpPrxOff_Click 0
            End If
            
            Dim sleepTime As Long, pOffset As Long, prox As Double, pram As Long
            
            ucALSusb1.dGetSleep sleepTime
            ucALSusb1.dSetSleep 5
            ucALSusb1.dSetEnable 1, 1
            ucALSusb1.dSetProxOffset 0
            Sleep 25: DoEvents
            ucALSusb1.dGetProximity prox
            
            i = 0
            Do While prox > 0.5
                pOffset = pTable(i): pram = pOffset
                ucALSusb1.dSetProxOffset pram
                Sleep 25: DoEvents
                ucALSusb1.dGetProximity prox
            ...
            Do While prox < 0.01 And pOffset > 0
                pOffset = pOffset - 1: pram = pOffset
                ucALSusb1.dSetProxOffset pram
                Sleep 25: DoEvents
                ucALSusb1.dGetProximity prox
            ...
            prox = prox + pram / 255
            cb177CmpPrxOff(Index).caption = "Offset=" & Int(prox * 100) / 100
            
            cb177CmpPrxOff(Index).value = vbUnchecked
            cb177CmpPrxOff(Index).enabled = True
...
Private Sub cmbIrdrMode_Click()
    Dim mode As Long
    mode = cmbIrdrMode.ListIndex
    ucALSusb1.dSetIrdrMode mode
End Sub
...
Private Sub tb58334aMux_Change()
    Dim v As Long
    On Error GoTo error
    
    If enterText(tb58334aMux.text) Then
        v = val(tb58334aMux.text)
        ucALSusb1.dSetEnable 0, v
        tb58334aMux.text = str(v)
    End If
    GoTo endSub
...
Private Sub tmrInit_Timer()
    ...
    Dim usb As String, partNumber As Long
    ...
    If usb <> "" Then
        ...
        ucALSusb1.dGetPartFamily gPartNumber
        ucALSusb1.dGetPartNumber partNumber
        If (29000 <= gPartNumber And gPartNumber <= 59000) Or gPartNumber = 2771 Then
            ...
            If gPartNumber = 29038 Then frm29038.Show 0, Me
            
            If partNumber = 29177 Then f29177.Visible = True
...
Private Sub txtIO_Change(Index As Integer)
    ...
    If enterText(txtIO(Index).text) Then
        If Index Then ' write field on data change
            ...
            ucALSusb1.dWriteField IOreg.a, IOreg.s, IOreg.m, d
        ...
        If cbEcho.value Or readEn Then
            ucALSusb1.dReadField IOreg.a, IOreg.s, IOreg.m, d
        End If
...
Private Sub tbI2Caddr_Change()
    Dim addr As Long
    If enterText(tbI2Caddr.text) Then
        addr = val("&H" & tbI2Caddr.text)
        'ucALSusb1.dWriteI2c &H50, 1, addr
        ucALSusb1.setI2cAddr (addr)
        tbI2Caddr.text = Hex(addr)
        'txtIO(0).text = txtIO(0).text & vbCr
    End If
End Sub

Private Sub cmbDev_Click()
    Const sync As Integer = &H30
    Const Din As Integer = 8
    Const Sclk As Integer = 4
    Const addr As Integer = 1
    
    Dim Dev As Integer: Dev = cmbDev.ListIndex
    Dim b10 As Integer: b10 = 3 * (Dev And &H10) / 16
    Dim d As Integer        ' data
    Dim c As Integer: c = 1 ' clk
    Dim s As Integer: s = 1 ' *sync
    Dim data As Byte
    
    ucALSusb1.setI2cAddr (&H3A)
    
    data = s * sync + c * Sclk: ucALSusb1.dWriteField addr, 0, &HFF, data: s = s Xor 1
    data = s * sync + c * Sclk: ucALSusb1.dWriteField addr, 0, &HFF, data: c = c Xor 1 ' sync low
    data = s * sync + c * Sclk: ucALSusb1.dWriteField addr, 0, &HFF, data: s = s Xor 1
    data = s * sync + c * Sclk: ucALSusb1.dWriteField addr, 0, &HFF, data: c = c Xor 1 ' sync hi
    
    For d = 5 To 0 Step -1
        data = s * sync + Din * ((Dev And 2 ^ d) / (2 ^ d)) + Sclk
        ucALSusb1.dWriteField addr, 0, &HFF, data
        Debug.Print DecToBin(data)
        data = data Xor Sclk
        ucALSusb1.dWriteField addr, 0, &HFF, data
        Debug.Print DecToBin(data)
    Next d
...
Public Function DecToBin(ByVal DeciValue As Long, Optional NoOfBits As Integer = 8) _
As String
'********************************************************************************
'* Name : DecToBin
'* Date : 2003
