...
Module usJungoUsb
	...
	Private Sub waitForCallBack(Optional ByRef noMsg As Boolean = False)
		...
		For i = 0 To TimeOut
			If (m_flag.DeviceAttached) Then
				Exit For
			Else
				System.Windows.Forms.Application.DoEvents() : Sleep((1))
			End If
		Next i
	...
	Private Sub openDevice()
        ...
        ' JWG		Call SetEventTable(eventTable, AddressOf USBattachCallback, AddressOf USBdetachCallback)
		'UPGRADE_ISSUE: VarPtr function is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
        ' JWG		eventTable.pUserData = VarPtr(m_pUserData)
        ...
        ' JWG		dwError = getError(WDU_Init(m_hDriver, matchTable, 1, eventTable, DEFAULT_LICENSE_STRING, WD_ACKNOWLEDGE, gHwnd))
		...
		'UPGRADE_ISSUE: VarPtr function is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
        ' JWG		dwError = getError(WDU_GetDeviceInfo(m_pUserData.hDev, VarPtr(m_dwDevAddr)))
	...
	Public Sub closeDevice()
		...
		If (m_flag.DeviceOpened = True) Then
            ...
            ' JWG			getError(WDU_ResetDevice(m_pUserData.hDev, 0))
            ' JWG			WDU_Uninit(m_hDriver)
			WD_Close((m_hDriver))
