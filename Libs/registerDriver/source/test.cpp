...
#ifdef _WINDOWS
#pragma warning (disable: 4482) // nonstandard extension used: enum 't_status' used in qualified name
...
t_status CalsBase::testNotImplemented()
{
	CalsBase* p=this;
...
t_status CalsBase::testRegression()
{	// this table must be sync'd with devices defined in CresourceManager
	...
	uw testEnableTable[]={7,29011,29020,29023,29025,29028,29033,29200
	   ,regIdx::enable     ,    1,    1,    1,    1,    1,    1,    1
	   ,regIdx::intFlag    ,    1,    0,    1,    1,    1,    1,    1
	   ,regIdx::intPersist ,    1,    0,    1,    1,    1,    1,    1
	   ,regIdx::data       ,    1,    1,    1,    1,    1,    1,    1
	   ,regIdx::threshHi   ,    1,    0,    1,    1,    1,    1,    1
	   ,regIdx::threshLo   ,    1,    0,    1,    1,    1,    1,    1
	   ,regIdx::inputSelect,    1,    1,    1,    1,    1,    1,    1
	   ,regIdx::range      ,    1,    1,    1,    1,    1,    1,    1
	   ,regIdx::resolution ,    1,    1,    1,    1,    1,    1,    1
	   ,regIdx::irdr       ,    1,    0,    0,    0,    1,    0,    1
	   ,regIdx::runMode    ,    1,    1,    1,    1,    0,    1,    1
	   ,regIdx::proxAmbRej ,    1,    0,    0,    0,    0,    0,    1
	   ,regIdx::irdrFreq   ,    1,    0,    0,    0,    0,    0,    1
	   ,regIdx::sleep      ,    0,    0,    0,    0,    1,    0,    0
	   ,regIdx::intLogic   ,    0,    0,    0,    0,    1,    0,    0
	}
...
t_status CalsBase::testSetTableIndex(const uw rti,const uw v0)
{
	CalsBase* p=this;
	t_status s=driverError;
	uw c=m_regTable[2+rti*5],v=0;
	...
	case regIdx::intFlag:	 s=p->m_pIO->write(&m_reg->m_intFlag[c],v);break;
	case regIdx::intPersist: s=p->setIntPersist(c,v);break;
	case regIdx::data:	 	 s=p->m_pIO->write(&m_reg->m_data[c],v);break;
...
t_status CalsBase::testGetTableIndex(const uw rti,uw& v)
{
	CalsBase* p=this;
	t_status s=driverError;
...
t_status CalsBase::testRegisterXtalk()
{
	t_status s=ok;
	uw i,j,nReg=(m_regTable[0]-1)/5,v;
...
t_status CalsBase::testErrorChecking(uw devN,uw* testEnableTable)
{
	CalsBase* p=this;
	t_status status=ok;

	uw iv,lv,ic=p->m_Nchannels,lc=ic-1;// illegal/legal - value/channel
	...
	if testEnabled(regIdx::intFlag)         // IntFlag
	{
...
t_status CalsBase::test0()
{
	CalsBase* p=this;
	t_status status=ok;
	uw e,i,j,m=0,t,t0,t1,meas[3][50];
	...
	if (status==ok) status=p->setThreshHi((dbl)0);   // force interrupt
